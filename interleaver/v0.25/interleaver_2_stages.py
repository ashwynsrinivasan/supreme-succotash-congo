...
class interleaver_2_stage_calc(OptRow):
    def __init__(self,
        ...
        self.dc3 = DirectionalCoupler(splitratio=dc3, loss = SiN_directional_coupler_loss_dB)
    ...
    def minimize_cost_function(
        ...
        if random_dc == True:
            ...
            bounds.append(
                (self.dc3._splitratio - bounds_epsilon,self.dc3._splitratio + bounds_epsilon)
                )
    ...
    def calculate_spectrum(self, dc1, dc2, dc3, phi1, phi2):
        self.dc1._splitratio = dc1
        self.dc2._splitratio = dc2
        self.dc3._splitratio = dc3
    ...
    def save_result(self, result):
        with open("bidi_sweep_results_2_stage.csv", 'a') as f:
            dc1, dc2, dc3, phi1, phi2 = result.x
            final_cost_function_value = result.fun
            to_append_to_csv = np.array(
                [
                    dc1,
                    dc2,
                    dc3,
            ...
            df = pd.DataFrame(
                {
                    "DC1": to_append_to_csv[0],
                    "DC2": to_append_to_csv[1],
                    "DC3": to_append_to_csv[2],
                    "PHI1": to_append_to_csv[3],
                    "PHI2": to_append_to_csv[4],
                    "Final_cost": to_append_to_csv[5],
                },
                index=[0]
            )
