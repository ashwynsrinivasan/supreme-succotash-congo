...
def device_definition(no_iterations=int(1e4+1), no_mrms = 8, no_crrs = 8, no_resonances = 6, center_resonance = 3, no_laser_wavelengths = 8, centroid_die_variation_bool = False, laser_distribution_uniform=True, die_variation_uniform_bool=False):
  np.random.seed(123)
  mrm_resonances = np.arange(0, 1600, 200)
  crr_resonances = np.arange(0, 1600, 200)
  laser_frequency = np.arange(0, 3200, 400)

  mrm_fsr = np.arange(1780, 1825, no_resonances)
  mrm_fabrication_variation = 100
  mrm_fabrication_variation_sigma = 50
  mrm_optical_bandwidth = 100
  crr_fsr = np.arange(1780, 1825, no_resonances)
  crr_fabrication_variation = mrm_fabrication_variation
  crr_optical_bandwidth = 150
  ...
  if laser_distribution_uniform:
    if not centroid_die_variation_bool:
          if die_variation_uniform_bool: 
            ...
            mrm_die_variation = np.random.normal(0, mrm_fabrication_variation_sigma, size = (no_laser_wavelengths,no_iterations)) + np.random.normal(0, laser_variation, size = (no_laser_wavelengths,no_iterations))
    else:
      ...
      if die_variation_uniform_bool:
        ...
        mrm_die_variation_max = np.random.normal(0, mrm_fabrication_variation_sigma, size = (no_iterations)) + np.random.normal(0, laser_variation, size = (no_iterations))
    ...
    if not centroid_die_variation_bool:
      if not die_variation_uniform_bool:
