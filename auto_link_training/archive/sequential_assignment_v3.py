...
def sequential_assignment(device_resonances_iter, laser_frequency, count_idx_iter, device_optical_bandwidth, step_sweep = 200, debug = 0):
  ...
  for laser_no, laser_val in enumerate(laser_frequency):
    ...
    if debug:
      ...
      print("device_focus: ", device_focus)
      print("device_preheated: \n", device_preheated[device_focus,:])
      print("laser_search: ", laser_val, ", laser_no: ", laser_no)
      print("step_sweep: ", step_sweep)
      print("resonance_location: \n", resonance_location, step_sweep * 2)
    for res_no in range(device_resonances_iter.shape[1]):
      if (resonance_location[res_no] >= -resonance_laser_error/2) and (resonance_location[res_no] <= step_sweep*2+resonance_laser_error):
        if debug:
          print("Phase 0 is successful")
          print("device_focus: ", device_focus)
          print("device_preheated: \n", device_preheated[device_focus,:])
          print("laser_search: ", laser_val, ", laser_no: ", laser_no)
          print("step_sweep: ", step_sweep)
  ...
  if debug:
    ...
    print(device_assigned)
    print(laser_assigned)
    print(heat_assigned)
    print(resonance_assigned)
  ...
  for device_no in np.linspace(device_focus-1, device_focus-4, 4, dtype=int):
    device_locked = False
    ...
    for step in [6, -3]:
      if not device_locked:
        ...
        if debug:
          ...
          print("laser_search: ", laser_search)
          print("step_sweep: ", step_sweep)
...
def device_array_decision(device_assigned, no_devices, iter_no, preheat=0):
  ...
  count_idx = []
  if np.min(count) == 2:
    count_idx.append((iter_no, 0, -1, 2))
  else:
    for idx in range(no_devices):
      if count[no_devices - idx - 1] == 1:
        if (no_devices - idx - 1) < 4:
          count_idx.append((iter_no, preheat, no_devices - idx - 1, 1))
        if (no_devices - idx - 1) >= 4:
          count_idx.append((iter_no, preheat, no_devices - idx - 1, 1))
        break
    
    # reference_found = False
    # for idx in np.linspace(int(no_devices/2), no_devices-1, int(no_devices/2), dtype=int):
    #   if count[idx] == 1:
    #     count_idx.append((iter_no, preheat, idx, 1))
    #     reference_found = True
    #     break
    # if not reference_found:
    #   for idx in np.linspace(0, int(no_devices/2)-1, int(no_devices/2), dtype=int):
    #     if count[idx] == 1:
    #       count_idx.append((iter_no, preheat, idx, 1))
    #       reference_found = True
    #       break
