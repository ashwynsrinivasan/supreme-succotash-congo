...
for u_ind in range(3):
    ...
    for phi_ind in range(numOfPhis):
        ...
        code_ctrl = 0 * np.ones((4, dims))
        code_ps = 0 * np.ones((2, 4, dims))
        voltage_ps = 0 * np.ones((2, 4, dims))
        phi_ps = 0 * np.ones((2, 4, dims)) # (upper/lower branch ps, stage, time)

        uout_te = np.zeros((3,dims), dtype=complex)
        uout_tm = np.zeros((3,dims), dtype=complex)

        psout_up = np.zeros((4, dims), dtype=complex)
        psout_down = np.zeros((4, dims), dtype=complex)
        dcoupout_up = np.zeros((4, dims), dtype=complex)
        dcoupout_down = np.zeros((4, dims), dtype=complex)
        ...
        for ind in range(dims):
            ...
            if state == 1:
                code_ctrl[3, ind] = np.minimum(2**dacres - dcode, np.maximum(code_ctrl[3, ind-2] + code_update, -2**dacres))
            if state == 2:
            ...
            if state == 6:
                code_ctrl[2, ind] = code_ctrl[2, ind-1] + dcode
            if state == 7:
                code_ctrl[2, ind] = np.minimum(2**dacres - dcode, np.maximum(code_ctrl[2, ind-2] + code_update, -2**dacres))
            if state == 8:
                code_ctrl[3, ind] = code_ctrl[3, ind-1] + dcode
            ...
            det_up[ind] = dcoupout_up[3, ind] * np.conj(dcoupout_up[3, ind])
            det_down[ind] = dcoupout_down[3, ind] * np.conj(dcoupout_down[3, ind])

            # iadc conversion
            iadc_code[ind] = int(det_up[ind] / iref * 2**adcres)
        ...
        maxloss[u_ind, phi_ind] = 10 * np.log10(np.min(det_down[int(det_down.size/2):]))
