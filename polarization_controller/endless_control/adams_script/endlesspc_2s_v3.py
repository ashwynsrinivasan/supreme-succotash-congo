...
state = 1 #state
dvoltage = np.pi / 128 * 1.65 / np.pi # need to figure out a good way to set this
voltage_update = 0
...
for ind in range(dims):
    for snum in range(2):
        if state % 2 == 1:
            voltage_ctrl[snum, ind] = voltage_ctrl[snum, ind - 2]
        else:
            voltage_ctrl[snum, ind] = voltage_ctrl[snum, ind - 1]
    if state == 1:
        voltage_ctrl[1, ind] = voltage_ctrl[1, ind-2] + voltage_update
    if state == 2:
        voltage_ctrl[0, ind] = voltage_ctrl[0, ind-1] + dvoltage
    if state == 3:
        voltage_ctrl[0, ind] = voltage_ctrl[0, ind-2] + voltage_update
    if state == 4:
        voltage_ctrl[1, ind] = voltage_ctrl[1, ind-1] + dvoltage
    ...
    # convert voltage_ctrl to phi_ps
    for sn in range(2):
        if voltage_ctrl[sn, ind] >= 0:
            phi_ps[0, sn, ind] = vheat2phase(voltage_ctrl[sn, ind])
            phi_ps[1, sn, ind] = 0
        else:
            phi_ps[0, sn, ind] = 0
            phi_ps[1, sn, ind] = vheat2phase(-voltage_ctrl[sn, ind])
    ...
    if state % 2 == 0:
        if update_mode == 1:
            voltage_update = - mu1 * np.sign(det_down[ind] - det_down[ind-1]) * det_down[ind-1] - mu2 / np.pi * voltage_ctrl[int(state/2)-1, ind-1]
        elif update_mode == 1.1:
            voltage_update = - mu1 * np.sign(det_down[ind] - det_down[ind-1]) * det_down[ind-1] - mu2 * np.sin(1/1.65 * voltage_ctrl[int(state/2)-1, ind-1])
        elif update_mode == 1.2:
            voltage_update = - mu1 * np.sign(det_down[ind] - det_down[ind-1]) * det_down[ind-1] - mu2 * np.tan(1/1.65 * voltage_ctrl[int(state/2)-1, ind-1])
        elif update_mode == 1.3:
            voltage_update = - mu1 * np.sign(det_down[ind] - det_down[ind-1]) * det_down[ind-1] - mu2 / np.pi**3 * voltage_ctrl[int(state/2)-1, ind-1]**3
    if state < 4:
        state = state + 1
    else:
        state = 1
...
ax[2].plot(voltage_ctrl[0,], label='voltage_ctrl1')
ax[2].plot(voltage_ctrl[1,], label='voltage_ctrl2')
ax[2].legend()
ax[2].set_title('Phase shifter vheat')
ax[2].set_ylabel('Voltage')
ax[2].set_ylim(-1.65,1.65)

print(f'Min phase 1 = {np.min(voltage_ctrl[0,int(dims/2):])}')
print(f'Max phase 1 = {np.max(voltage_ctrl[0,int(dims/2):])}')
print(f'Min phase 2 = {np.min(voltage_ctrl[1,int(dims/2):])}')
print(f'Max phase 2 = {np.max(voltage_ctrl[1,int(dims/2):])}')
