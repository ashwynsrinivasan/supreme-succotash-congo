...
mu1 = 7 * vmax / np.pi * (2**dacres) / (vmax - vmin) / 2**adcres
mu2 = 0.4
# --------------------------------
...
for phi_ind in range(numOfPhis):
    ...
    phi_u[0] = input_phis[phi_ind] * np.ones(dims)
    phi_u[2] = input_phis[phi_ind] * np.ones(dims)
    ...
    maxloss[phi_ind] = 10 * np.log10(np.min(det_up[int(det_up.size/2):]))
