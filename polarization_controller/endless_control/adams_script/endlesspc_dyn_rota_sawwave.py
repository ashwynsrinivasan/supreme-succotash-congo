...
from scipy import signal # importing for riangle wave
...
nstages = 4
state = 1 #state
dcode = 2.3 * int(np.pi * vmax / 128 / np.pi * (2**dacres) / (vmax - vmin)) # need to figure out a good way to set this
code_update = 0
update_mode = 1 #(1 - linear limit function, 1.1 - sin, 1.2 - tan, 1.3 - cube
fclk = 16e3
print(f'dcode = {dcode}')
# Parameter for mode (1)
mu1 = 6 * (vmax / np.pi * (2**dacres) / (vmax - vmin) / 2**adcres)
mu2 = 0.01
# --------------------------------

# Sweep settings -----------------
slope = 500 # rad/s
dc_phase = np.pi / 2 # rad
u_ind = 2
...
phi_u[u_ind, ] = signal.sawtooth(slope * time, 0.5) * np.pi / 2 + dc_phase
