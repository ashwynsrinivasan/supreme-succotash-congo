...
for mu1_ind in range(mu1N):
    for mu2_ind in range(mu2N):
        ...
        maxloss[mu1_ind, mu2_ind] = 10 * np.log10(np.min(max_det[int(3*max_det.size/4):]))
...
ax[0].legend()
ax[0].set_title(f'Input rotation ({in_rot:.2f} rads)')
...
contour_plot = plt.contour(mu1_mesh, mu2_mesh, -maxloss, norm=colors.LogNorm(),levels=contour_levels, cmap='turbo')
...
plt.xlabel('mu1')
plt.ylabel('mu2')
plt.title(f'Max loss contour plot for agressor (U3 = {u_ind+1}, Rot. angle = {in_rot:.2f} rad/s)')
