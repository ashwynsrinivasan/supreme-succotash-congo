...
dims = 100
pow = iref
input_te_pow = np.ones(dims) * np.sqrt(pow)
...
#phi_u[2] = np.linspace(0,2*np.pi,dims) * np.ones(dims)
phi_u[2] = np.pi * np.ones(dims)
...
mu2 = 0.1 / (vmax / np.pi * (2**dacres) / (vmax - vmin) / 2**adcres)

print(f'mu1 = {mu1}')
print(f'mu2 / pi = {mu2/np.pi}')
print(f'Coefficient ratio = {mu2 / np.pi / mu1}')
...
maxloss = 10 * np.log10(np.min(det_up[int(det_up.size/2):]/pow))
...
ax.plot(code_ctrl[0,], label='code_ctrl1')
ax.plot(code_ctrl[1,], label='code_ctrl2')
ax.legend()
ax.set_title('Phase shifter HDAC ctrl code')
ax.set_ylabel('Code')
ax.set_ylim(-2**(dacres+0.5), 2**(dacres+0.5))
ax.axhline(y=2**dacres, color='r', linestyle='--')
ax.axhline(y=-2**dacres, color='r', linestyle='--')
ax.grid()
