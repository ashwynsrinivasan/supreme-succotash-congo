...
class fiber_shuffle(OptElement):
  ...
  def _construct_smatrix(self):
    ...
    if self._return_loss + self._te_loss > 0:
      print('Warning: return loss + TE loss > 0, proof of concept model not valid')

    if self._return_loss + self._tm_loss > 0:
      print('Warning: return loss + TM loss > 0, proof of concept model not valid')
...
class optical_connector(OptElement):
  ...
  def __init__(
    ...
    return_loss: Union[float, Callable] = -100,
    ports: Tuple[str, str, str, str] = ('1l', '2r', '3r', '4l'),
    name: str = 'CONNECTOR'
  ):
  ...
  def _construct_smatrix(self):
    ...
    if self._loss + self._return_loss > 0:
      print('Warning: loss + return loss > 0, proof of concept model not valid')
...
class polarization_splitter_rotator(OptElement):
  ...
  def __init__(
    ...
    TM_phase: Union[float, Callable] = np.pi/2,
    ports: Tuple[str, str, str, str] = ('1l', '2r', '3r', '4l'),
    name: str = 'PSR'
  ):
    self._te_loss = self._genvar(TE_loss)
  ...
  def _construct_smatrix(self):
    ...
    if self._te_loss + self._te_per > 0:
      print('Warning: TE loss + TE PER > 0, proof of concept model not valid')

    if self._tm_loss + self._tm_per > 0:
      print('Warning: TM loss + TM PER > 0, proof of concept model not valid')
...
class polarization_controller_single_fiber(OptRow):
  def __init__(
    self, 
    psr_bool = True,
    mzi_bool = False,
    tx_num_mzi_stages = 2,
    rx_num_mzi_stages = 1,
    name = 'POLARIZATION_CONTROLLER_SINGLE_FIBER',
  ):
...
class polarization_controller_double_fiber(OptRow):
  def __init__(
    self,
    psr_bool = True,
    mzi_bool = False,
    tx_num_mzi_stages = 2,
    rx_num_mzi_stages = 1,
    name = 'POLARIZATION_CONTROLLER_DOUBLE_FIBER',
  ):
    self.bidi_tx = bidi_tx(num_mzi_stages = tx_num_mzi_stages)
    self.fiber_1 = optical_fiber()
...
class polarization_controller(OptRow):
  def __init__(
    self,
    psr_bool = True,
    mzi_bool = False,
