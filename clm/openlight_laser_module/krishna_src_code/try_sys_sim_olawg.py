...
N_array = 3
l = laser(tj=Th+5,i=200e-3, N_array=N_array, delta_T=0.1)
m = awg('pcm/congo/vernier_laser/krishna_src_code/lightmatter_cyclic.csv')
m.tune(l.lambda_array0[:,0]*1e9)
...
pid_heater = [pid(kp=0.1, ki=1/(10*dt), kd=0.000, dt=dt) for i in range(N_array)]
if not is_heater:
    pid_laser_drive = [pid(kp=0.1, ki=1/(10*dt), kd=0, dt=dt) for i in range(N_array)]
else:
    pid_laser_drive = [pid(kp=0.1, ki=0.00, kd=0.000, dt=dt) for i in range(N_array)]
...
for step in range(len(t)):
    ...
    if step<len(t)-1 and t[step]>=30:
        ...
        power_err_array = p_optical[step, :] - Plaser_ref
        q[step+1,0] = sum(p_loss)
        p_mux_out[step, :] = m.update(lambda_array_step*1e9, p_optical[step, :])
        err_mux_out = Pmux_ref - p_mux_out[step, :]
        if t[step] >= 40:

            for i in range(N_array):
                pid_heater[i].update(err_mux_out[i])
                pid_laser_drive[i].update(err_mux_out[i])
                if is_heater:
                    I_heater_array[step+1, i] = pid_heater[i].output
                elif not is_heater:
                    if pid_laser_drive[i].output > 0:
                        I_laser_array[step+1, i] = 0.25 - pid_laser_drive[i].output
                    else:
                        I_laser_array[step + 1, i] = 0.25 + pid_laser_drive[i].output

        if t[step] >= 50:
            print('step: ', step, 'T: ', Ta[step,0]-273, 'I: ', I[step,0], 'q: ', q[step,0], 'Pmux: ', p_mux_out[step, :]*1e3, 'lambda_err: ', lambda_err_array[step, :])
...
if type(m) is mux:
    ...
    for i in range(len(m.channel_array)):
        plt.plot(m.lambda_sweep, m.il_dict[m.channel_array[i]])
        #Add a marker for the channel wavelength
        plt.plot(m.lambda_tuning_array[i], 10*np.log10(m.power_array[i]), 'r*')
